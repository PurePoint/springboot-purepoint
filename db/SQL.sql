...
(
    ...
    nickname        VARCHAR(100) NOT NULL,              -- 사용자 이름
    ...
    delete_at       DATETIME DEFAULT CURRENT_TIMESTAMP, -- 회원 탈퇴일
...
(
    ...
    post_delete_at DATETIME,                            #게시글 삭제일
...
(
    comment_id        CHAR(36) AUTO_INCREMENT PRIMARY KEY, #댓글 ID
    user_id           VARCHAR(255),                        #댓글 작성자 ID
    comment_contents  VARCHAR(255),                        #댓글 내용
    comment_update_at DATETIME,                            #댓글 수정일
    comment_delete_at DATETIME,                            #댓글 삭제일
    depth             INT,                                 #댓글 뎁스
    FOREIGN KEY (user_id) REFERENCES User (user_id)
);
